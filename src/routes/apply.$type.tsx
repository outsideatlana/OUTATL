...
export type AppType = "intern" | "freelancer" | "vendor" | "artist";

const META: Record<AppType, { title: string; tagline: string; num: string; roles: string[] }> = {
  intern: {
    title: "Intern Application",
    tagline:
      "Students and early-career applicants in event planning, marketing, social, content, ops, promotions, photo, video, hospitality, and artist relations.",
    num: "01",
    roles: [
      "Event Planning",
      "Marketing",
  ...
  vendor: {
    ...
    tagline:
      "Food, drink, clothing, merch, local brands, sponsors, and pop-up shops looking to activate at OutsideAtl events.",
    num: "03",
  ...
  artist: {
    ...
    tagline:
      "DJs, performers, producers, and artists — submit for booking, showcases, collaborations, and lineups.",
    num: "04",
...
/*
  beforeLoad: ({ params }) => {
    legacy param guard removed during Vite migration.
  },
  head: ({ params }) => {
...
export default function ApplyPage({ type }: { type: AppType }) {
  const t = type as AppType;
  const meta = META[t];
  const m = useAsyncMutation<Record<string, unknown>>((input) => submitApplication(input), {
    onSuccess: () => {
        ...
        <form onSubmit={onSubmit} className="max-w-3xl space-y-4">
          ...
          {isArtist && (
            <Field label="Stage name *">
              <Input required value={form.stage_name} onChange={set("stage_name")} />
            </Field>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Field label="TikTok">
              <Input placeholder="@handle" value={form.tiktok} onChange={set("tiktok")} />
            </Field>
          </div>
          <Field label="Portfolio / work samples (URL)">
            <Input
              type="url"
              placeholder="https://..."
          ...
          {isArtist && (
            <>
              ...
              <Field label="Preferred event type">
                <Input
                  placeholder="Rave, day party, festival, showcase..."
