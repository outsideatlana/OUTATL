...
const HEAD = {
  ...
  description:
    "Brands, agencies, and partners — sponsor an OutsideAtl event. Pick a package or pitch a custom collaboration.",
};
    ...
    <div className="min-h-screen text-foreground">
      ...
      <section className="relative px-6 py-20 md:py-28 gradient-divider overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl">
            Activate your brand alongside Atlanta's after-hours. Pick a package below or pitch a
            custom collab — we tailor every partnership to your goals.
          </p>
        </div>
      </section>
      ...
      <section className="relative px-6 py-16">
        <form onSubmit={onSubmit} className="max-w-3xl space-y-4">
          ...
          <h2 className="font-mono text-xs uppercase tracking-widest text-accent pt-6">
            [ Primary Contact ]
          </h2>
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Field label="Title / Role">
              <Input
                placeholder="Marketing Manager, Founder..."
          ...
          <Field label="Instagram">
            <Input placeholder="@brand" value={form.instagram} onChange={set("instagram")} />
          </Field>
          ...
          {form.package === "Custom — let's talk" && (
            <Field label="Describe your custom ask *">
              <Textarea
                required
                rows={3}
                placeholder="What kind of collab do you have in mind?"
                value={form.custom_ask}
                onChange={set("custom_ask")}
              />
            </Field>
          )}
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Budget range">
              <Input
                placeholder="$2k–$5k, flexible, in-kind, etc."
                value={form.budget}
                onChange={set("budget")}
              />
            </Field>
            <Field label="Preferred event type">
              <Input
                placeholder="Rave, day party, festival..."
          ...
          <Field label="Goals for this partnership *">
            <Textarea
              required
              rows={4}
              placeholder="Brand awareness, product sampling, leads, content, community, launch..."
              value={form.goals}
              onChange={set("goals")}
            />
          </Field>
          <Field label="Timing / availability">
            <Input
              placeholder="Q1 2026, next 3 months, specific event date..."
          ...
          <Field label="Preferred way to be contacted">
            <Select
              value={form.outreach_preference}
              onChange={set("outreach_preference")}
              options={OUTREACH}
            />
          </Field>
